...
def model_adaboost_train():
    ...
    pm25_data = pd.DataFrame(data, columns=[str(i) for i in range(0, weidu_size)])
    gbm0 = AdaBoostRegressor(DecisionTreeRegressor(max_depth=5, min_samples_split=20, min_samples_leaf=5),
                             learning_rate=0.04, n_estimators=500)
    gbm0.fit(train[important_column], y)
    ...
    for i in range(750, 1200):
        ...
        pred_PM25 = gbm0.predict(pm_data[important_column])
    ...
    # 模型存储
    joblib.dump(gbm0, './data/save_model/adaboost_best_yishen_buzhi3.pkl')

def model_adaboost_predict():
    f = h5py.File(
        './data/slice_eve_data/the_last_all__7_13_pca_com_800.h5',
        'r')
    # f['data'].value存放的是时间戳 上空间的流量数据
    ...
    adaboost_model = joblib.load(
        './data/save_model/adaboost_best_yishen_buzhi3.pkl')
    pm25_data = pd.DataFrame(data, columns=[str(i) for i in range(0, weidu_size)])
    ...
    for i in range(750, 1200):
        ...
        pred_PM25 = adaboost_model.predict(pm_data[important_column])
